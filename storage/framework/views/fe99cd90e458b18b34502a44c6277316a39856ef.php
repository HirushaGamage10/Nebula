...
document.addEventListener('change', function(e) {
    if (e.target.id === 'location') {
        window.location = '<?php echo e(route('payment.plan')); ?>?location=' + e.target.value;
...
$('#course, #location').on('change', function() {
    ...
    if($('#course').val() && $('#location').val()) {
        ...
        $.ajax({
            url: '<?php echo e(route("intakes.byCourse")); ?>',
            method: 'POST',
            data: {
            ...
            success: function(response) {
                if(response.success && response.data && response.data.length > 0) {
                    ...
                    response.data.forEach(function(intake) {
                        // Use batch directly from response
                        $('#intake').append(`<option value="${intake.intake_id}">${intake.batch}</option>`);
                    });
            ...
            error: function(xhr, status, error) {
                ...
                $('#intake').empty().append('<option selected disabled value="">Error loading intakes</option>');
                $('#intake').prop('disabled', true);
...
// Consolidated form field toggling on page load
window.addEventListener('DOMContentLoaded', function () {
    ...
    function toggleFormFields() {
        ...
        if (!locationSelected) {
            // Disable all form fields when no location is selected
...
<?php echo $__env->make('inc.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\thisali\Desktop\thisali\Nebula\resources\views/payments/payment_plan.blade.php ENDPATH**/ ?>