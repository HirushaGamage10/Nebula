...
document.getElementById('filterForm').addEventListener('submit', async e => {
  ...
  const res = await fetch('<?php echo e(route("student_management.filter")); ?>', {
    method: 'POST',
...
document.getElementById('student_id').addEventListener('change', async e => {
  ...
  if (!studentId) {
    ...
    <?php $__currentLoopData = \App\Models\Course::orderBy('course_name')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      document.getElementById('courseSelect').innerHTML += '<option value="<?php echo e($course->course_id); ?>"><?php echo e($course->course_name); ?></option>';
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    return;
  }

  try {
    const res = await fetch('<?php echo e(route("student_management.courses")); ?>?student_id=' + encodeURIComponent(studentId));
    const data = await res.json();
...
<?php echo $__env->make('inc.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\thisali\Desktop\thisali\Nebula\resources\views/student_management/student_view.blade.php ENDPATH**/ ?>